...
* `export()` gains `template_params` and `template_dir` arguments to control the export template HTML files, allowing users to customize the exported HTML files. The export template is provided by the shinylive assets and may change from release-to-release. Use `assets_info()` to locate installed shinylive assets; the template files for a given release are in the `export_template` directory of the release. (#96)
